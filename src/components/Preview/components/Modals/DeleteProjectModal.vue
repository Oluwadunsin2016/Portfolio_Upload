<template>
      <transition name="deleteProjectModal">
   ...
   <div
    ...
    <div class="mb-4">
    <h1 class="text-lg">Are you sure you want to delete this Project?</h1>
    </div>
...
created(){
window.emitter.on('showDeleteProjectModal',(result)=>{
...
this.isOpened=false
 ...
 this.loading=true
  axios
          .delete(
            `${baseURL}deleteProject/${this.project.id}`,config
          )
          .then((res) => {
...
<style scoped>
.deleteProjectModal-enter-from,.deleteProjectModal-leave-to{
opacity:0;
transform:scale(0.1)
}

.deleteProjectModal-enter-active,.deleteProjectModal-leave-active{
