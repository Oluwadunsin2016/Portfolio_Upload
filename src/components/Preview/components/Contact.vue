<template>
  <div
    id="contact"
    class="px-4 md:px-6 lg:px-8 py-16 bg-slate-100 dark:bg-gray-800 rounded-md"
  >
        ...
        <ul class="px-2">
          <li class="my-4 flex gap-4 items-center">
            ...
            <div>
              ...
              <a :href="`mailto:${user.email}`" class="gmail">{{
                user.email
              }}</a>
            </div>
          </li>
...
import EditSocialModal from './Modals/EditSocialModal.vue'
...
export default {
  ...
  created() {
  axios
        .get(`${baseURL}getSocialInformation/${this.user.info_token}`)
        .then((res) => {
  ...
  methods: {
    ...
    submitInformation() {
      ...
      this.loading=true
    axios
        .post(`${baseURL}sendMail/${this.user.info_token}`, this.information)
        .then((res) => {
          console.log(res);
          this.loading=false
              if(!res.data.error){
    ...
    socialEdit(){
    // this.editMode=true
    window.emitter.emit('showEditSocialModal',{information:this.social_information,status:true})
    }
  },
};
</script>
