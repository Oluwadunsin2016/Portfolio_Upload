<template>
  <div class="h-[100vh] flex justify-center items-center">
    <div
      class="relative overflow-hidden min-h-[15rem] w-full sm:w-[60%] md:w-[40%] p-4 md:p-8 shadow-md bg-slate-100 dark:bg-gray-800 dark:shadow-gray-850 rounded-md mx-4"
    >
      <div
        v-if="isDone"
        class="bg-gray-400 dark:bg-gray-800 bg-opacity-70 dark:bg-opacity-80 transition-all ease-in-out absolute top-0 left-0 w-full h-full flex p-4 items-center justify-center"
      >
        <svg
          v-if="loading"
        ...
        >
          <linearGradient id="a12">
            <stop offset="0" stop-color="#D3E3FD" stop-opacity="0"></stop>
            <stop offset="1" stop-color="#D3E3FD"></stop>
          </linearGradient>
          <circle
            fill="none"
            stroke="url(#a12)"
      ...
      <div v-else class="mb-4">
        ...
        <button
          @click="reSend"
          v-if="expired"
          class="cursor-pointer text-gray-600 dark:text-gray-400 underline"
        >
          Send again
        </button>
...
handleInterval(){
    ...
    verify(number) {
      this.loading = true;
      axios
        .post(`${baseURL}verifyCode`, { number })
        .then((res) => {
          if (!res.data.error) {
            this.loading = false;
    ...
    reSend() {
      let email = "";
      window.emitter.on("email", (sentEmail) => {
        email = sentEmail;
      });
      this.minutes= 5
      this.seconds= 0
      ...
      axios
        .post(`${baseURL}forgottenPassword`, { email })
        .then((res) => {
          console.log(res);
          if (res.data.error) {
